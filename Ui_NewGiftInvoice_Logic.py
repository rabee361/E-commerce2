...
from Ui_NewGiftsInvoice import Ui_NewGiftInvoice


class Ui_NewGiftInvoice_Logic(object):
    def __init__(self, sqlconnector):
        super().__init__()
        self.sqlconnector = sqlconnector
        self.database_operations = DatabaseOperations(sqlconnector)
        self.ui = Ui_NewGiftInvoice()

    def showUi(self):
        window_newgiftinvoice = QDialog()
        self.ui.setupUi(window_newgiftinvoice)
        self.initialize(window_newgiftinvoice)
        window_newgiftinvoice.exec()
    ...
    def setProductFields(self):
        ...
        if (str(type(product_data)) != "<class 'NoneType'>"):
            ...
            product_price_after_discount_sp = round(
                float(float(product_unit_price_sp) * (100 - float(product_discount)) / 100),
                6)
