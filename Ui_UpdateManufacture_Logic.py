...
from Ui_UpdateManufacture import Ui_UpdateManufacture 

class Ui_UpdateManufacture_Logic(QDialog):
    database_operations = ''
    ...
    def __init__(self, sql_connector, manufacture_process_id='',manufacture_process=''):
        super().__init__()
        self.manufacture_process = manufacture_process
    ...
    def showUi(self):
        ...
        window_manufacture.setWindowFlags(Qt.Window | Qt.WindowMaximizeButtonHint | Qt.WindowCloseButtonHint)
        ...
        if self.manufacture_process:
            window_manufacture.setWindowTitle(self.language_manager.translate('UPDATE_MANUFACTURE_PROCESS') + f' {self.manufacture_process}')
        else:
            window_manufacture.setWindowTitle(self.language_manager.translate('CREATE_MANUFACTURE_PROCESS'))
    ...
    def initialize(self, window):
        ...
        self.ui.remove_material.clicked.connect(lambda: self.fetchComposition())
        self.ui.remove_material.clicked.connect(lambda: self.fetchMaterialMachines())
        self.ui.remove_material.clicked.connect(lambda: self.setAllowedUnits())
    ...
    def addProducedMaterial(self):
        try:
            ...
            if not material_data:
                self.ui.status_bar_label.setText("⚠️ لا توجد مادة منتجة محددة")
                return
            
            amount = self.ui.produced_quantity1_input.text()
            if not amount:
                self.ui.status_bar_label.setText("⚠️ لا توجد كمية مصنعة محددة")
                return
    ...
    def fetchMaterials(self):
        ...
        try:
            ...
            error_message = f"{self.language_manager.translate('ALERT_ERROR_FETCHING_MATERIALS')}: {str(e)}"
            self.ui.status_bar_label.setText(error_message)
    ...
    def fetchComposition(self):
        ...
        for product_id in produced_material_ids:
            ...
            if not ingredients:
                warning = f"{self.language_manager.translate('ALERT_NO_COMPOSITION_FOR_PRODUCT')}: {product_id}"
                continue
        ...
        for material_id, ingredient in all_ingredients.items():
            ...
            if not material_warehouses:
                warning = f"{self.language_manager.translate('ALERT_NO_WAREHOUSES_FOR_MATERIAL')}: {ingredient['material_name']}"
                continue
                    ...
                    if self.ui.material_exact_invoice_radio.isChecked():   
                        ...
                        if invoice_item_data:
                            ...
                            if not invoice_item_unit_price:
                                warning = f"{self.language_manager.translate('ALERT_NO_EXCHANGE_RATE_FOR_MATERIAL')}: {ingredient['material_name']}"
                                return
                        ...
                        if payment_currency == manufacture_currency:
                            ...
                            if exchange_value:
                                ...
                                if exchange_value:
                                    ...
                                    warning = self.language_manager.translate('ALERT_NO_EXCHANGE_RATE_FOR_INVOICE_ITEM_AND_MANUFACTURE_CURRENCY')
                                    self.ui.composition_tree.clear()
                                    continue
        ...
        for i in range(produced_materials_count):
            ...
            if material_id and material_id != '':
                try:
                    ...
                    error_message = f"{self.language_manager.translate('ALERT_ERROR_FETCHING_MACHINES')}: {material_name}: {str(e)}"
                    self.ui.status_bar_label.setText(error_message)
                    continue
                ...
                for machine in machines:
                    ...
                    account_id = machine['account_id'] or ''
                    account_name = machine['account_name'] or ''
                    opposite_account_id = machine['opposite_account_id'] or ''
                    opposite_account_name = machine['opposite_account_name'] or ''
                    ...
                    machine_usage_details[machine_id].append((machine_id, machine_name, mode_id, mode_name, usage_duration, exclusive))
        ...
        for product_id in produced_material_ids:
            ...
            if index_with_value_on_both_lists is not None:
                try:
                    ...
                    self.ui.status_bar_label.setText(self.language_manager.translate('ALERT_ERROR_CALCULATING_PRODUCED_QUANTITY'))
                    return

        if not fraction:
            self.ui.status_bar_label.setText(self.language_manager.translate('ALERT_NO_PRODUCED_QUANTITY_DEFINED'))
            return
        else:
            ...
            if str(expenses_type).lower() == "real":  # real machines operation costs and salaries cost
                ...
                for row in range(self.ui.machines_table.rowCount()):
                    ...
                    usage_duration = self.ui.machines_table.item(row, 4).text()
                    exclusive = self.ui.machines_table.item(row, 5).text()
                    usage_percentage = self.ui.machines_table.item(row, 6).text()
                    ...
                    for cost in mode_costs:
                        ...
                        try:
                            consumption_per_usage = float(consumption_per_hour) * float(usage_duration)
                            if str(exclusive).lower() == 'no':
                                consumption_per_usage = consumption_per_usage * float(usage_percentage) / 100

                            cost_per_usage = float(cost_per_minute) * float(usage_duration) * 60
                        except ValueError:
                        ...
                        if currency_id != manufacture_currency:
                            ...
                            if exchange_value:
                                ...
                                win32api.MessageBox(0, self.language_manager.translate('ALERT_NO_EXCHANGE_RATE_FOR_MACHINE_COST'), "تنبيه")
                        ...
                        if resource_account_id in self.machines_operation_costs_dict:
                            ...
                            self.machines_operation_costs_dict[resource_id] = [cost_per_usage, manufacture_currency,
                                                                               resource_account_id, resource_name,
                                                                               resource_account_id, machine_name]
            ...
            ##########################################################################################################
                ...
                if not work_hours:
                    self.ui.status_bar_label.setText(self.language_manager.translate('ALERT_NO_WORK_HOURS_DEFINED'))
                else:
                    try:
                        ...
                        if standard_work_hours_per_day == 0 or '':
                            self.ui.status_bar_label.setText(self.language_manager.translate('ALERT_NO_WORK_HOURS_PER_DAY_DEFINED'))
                        ...
                        for data in payrolls_details:
                            ...
                            if currency_id != manufacture_currency:
                                ...
                                if exchange_value:
                                    ...
                                    win32api.MessageBox(0,self.language_manager.translate('ALERT_NO_EXCHANGE_RATE_FOR_SALARIES'),self.language_manager.translate('ALERT'))
                            ...
                            try:
                                ...
                                self.ui.status_bar_label.setText(self.language_manager.translate('ALERT_ERROR_CALCULATING_SALARIES_COST'))
                                return
            ...
            ##########################################################################################################
                ...
                if quantity_expenses:
                    quantity_data = self.database_operations.fetchQuantityOfManufacturedMaterials(
                        unit=quantity_expenses_target_unit, year=year, month=month)
                    if quantity_data:
                ...
                if expenses_data:
                    ...
                    if currency != manufacture_currency:
                        ...
                        if exchange_value:
                            ...
                            response = win32api.MessageBox(0,self.language_manager.translate('ALERT_NO_EXCHANGE_RATE_FOR_EXPENSES'),self.language_manager.translate('ALERT'), 4)
                            if response == 6: # Yes
                                ...
                                if latest_exchange_value:
                                    ...
                                    win32api.MessageBox(0,self.language_manager.translate('ALERT_NO_EXCHANGE_RATE_BETWEEN_CURRENCIES'),self.language_manager.translate('ALERT'))
                                    self.ui.composition_tree.clear()
                                    return
                            else: # No
                                self.ui.composition_tree.clear()
                ...
                if expenses and expenses_divider:
                    try:
                        ...
                        self.ui.status_bar_label.setText(self.language_manager.translate('ALERT_ERROR_CALCULATING_EXPENSES'))
                        return
        ...
        if manufactured_rows > 0:
            ...
            if machines_rows:
                for row in range(machines_rows):
                    ...
                    account_id = self.ui.machines_table.item(row, 7).text()
                    account_name = self.ui.machines_table.item(row, 8).text()
                    opposite_account_id = self.ui.machines_table.item(row, 9).text()
                    opposite_account_name = self.ui.machines_table.item(row, 10).text()
            ...
            messagebox_result = win32api.MessageBox(None, self.language_manager.translate('ALERT_SAVE_MANUFACTURE_PROCESS'), self.language_manager.translate('ALERT'), MB_YESNO)
            if (messagebox_result == IDYES):
                self.database_operations.removeManufactureProcessData(self.manufacture_process_id, commit=False)
                self.database_operations.updateManufactureProcessData2(self.manufacture_process_id,produced_materials, manufacture_date, working_hours, ingredients_pullout_date, expenses_cost, manufactue_currency, input_warehouse, account, mid_account, mid_account_input, batch_number, expenses_type, expenses_distribution, material_pricing_method, composition_materials_cost, machines_operation_cost, salaries_cost, quantity_unit_expenses, ingredients_pullout_method, ingredients_pullout_account, composition_data, machines_data, production_date, commit=False)


                if self.ui.generate_journal_entry.isChecked() and self.manufacture_process_id:
                    self.saveJournalEntry(self.manufacture_process_id, commit=False)
                if self.ui.add_material_to_warehouse_checkbox.isChecked() and self.manufacture_process_id:
                    self.addToWarehouse(self.manufacture_process_id,manufactue_currency)
    ...
    def viewSavedProcess(self):
        manufacture_data = self.database_operations.fetchManufactureProcess(self.manufacture_process_id)
        if manufacture_data:
            id = manufacture_data['id']
            pullout_date = manufacture_data['pullout_date']
            date = manufacture_data['date_col']
            expenses_type = manufacture_data['expenses_type']
            material_pricing_method = manufacture_data['material_pricing_method']
            currency = manufacture_data['currency']
            currency_name = manufacture_data['currency_name']
            expenses_cost = manufacture_data['expenses_cost']
            machines_operation_cost = manufacture_data['machines_operation_cost']
            salaries_cost = manufacture_data['salaries_cost']
            mid_account = manufacture_data['mid_account']
            mid_account_input = manufacture_data['mid_account_input']
            account = manufacture_data['account']
            composition_materials_cost = manufacture_data['composition_materials_cost']
            quantity_unit_expenses = manufacture_data['quantity_unit_expenses']
            expenses_distribution = manufacture_data['expenses_distribution']
            state_col = manufacture_data['state_col']
            factory_id = manufacture_data['factory_id']
            ingredients_pullout_method = manufacture_data['ingredients_pullout_method']
            ingredients_pullout_account = manufacture_data['ingredients_pullout_account']

            # self.ui.ingredients_pullout_date_input.setDate(pullout_date) TODO
            self.ui.ingredients_pullout_date_input.setEnabled(False)
            if isinstance(date, str):
                self.ui.manufacture_date_input.setDate(datetime.strptime(date, '%Y-%m-%d').date())
            else:
                self.ui.manufacture_date_input.setDate(date)
            self.ui.manufacture_date_input.setEnabled(False)

            self.ui.currency_combobox.setCurrentIndex(self.ui.currency_combobox.findData(currency))
            self.ui.radio_quantity_unit_expenses_combobox.setCurrentIndex(
                self.ui.radio_quantity_unit_expenses_combobox.findData(quantity_unit_expenses))
            self.ui.expenses_type_combobox.setCurrentIndex(self.ui.expenses_type_combobox.findData(expenses_type))
            self.ui.mid_account_combobox.setCurrentIndex(self.ui.mid_account_combobox.findData(mid_account))
            self.ui.account_combobox.setCurrentIndex(self.ui.account_combobox.findData(account))
            self.ui.mid_account_input_combobox.setCurrentIndex(self.ui.mid_account_input_combobox.findData(mid_account_input))
            self.ui.ingredients_pullout_account_combobox.setCurrentIndex(self.ui.ingredients_pullout_account_combobox.findData(ingredients_pullout_account))
            self.ui.ingredients_pullout_method_combobox.setCurrentIndex(self.ui.ingredients_pullout_account_combobox.findData(ingredients_pullout_method))
            for attr_name in dir(self.ui):
                if expenses_distribution in attr_name or material_pricing_method in attr_name: # TODO
                    child_variable = getattr(self.ui, attr_name)
                    if child_variable:
                        child_variable.setChecked(True)

            # Fetch produced materials
            produced_materials = self.database_operations.fetchManufactureProcessProducedMaterials(self.manufacture_process_id)
            self.ui.produced_materials_tree.clear()
            for produced_material in produced_materials:
                material_id = produced_material['material_id']
                material_code = produced_material['code']
                material_name = produced_material['material_name']
                quantity1 = produced_material['quantity1']
                unit1 = produced_material['unit1']
                unit1_name = produced_material['unit1_name']
                quantity2 = produced_material['quantity2']
                unit2 = produced_material['unit2']
                unit2_name = produced_material['unit2_name']
                quantity3 = produced_material['quantity3']
                unit3 = produced_material['unit3']
                unit3_name = produced_material['unit3_name']
                working_hours = produced_material['working_hours']
                batch = produced_material['batch']
                referential_quantity1 = produced_material['referential_quantity1']
                referential_quantity2 = produced_material['referential_quantity2']
                referential_quantity3 = produced_material['referential_quantity3']
                referential_working_hours = produced_material['referential_working_hours']

                # Create a parent item for the material
                parent_item = QTreeWidgetItem([
                    str(material_id) if material_id else '',
                    str(material_code) if material_code else '',
                    str(material_name) if material_name else '',
                    '', '', '', '', '', '', '', '', ''
                ])

                # Set light blue background for material id and name
                for i in range(2):
                    parent_item.setBackground(i, QtGui.QColor("#E6F3FF"))

                # Create a child item for referential quantities
                ref_child = QTreeWidgetItem([
                    'Referential',
                    '',
                    str(round(float(referential_quantity1), int(self.float_point_precision))) if referential_quantity1 else '',
                    str(unit1) if unit1 else '',
                    str(unit1_name) if unit1_name else '',
                    str(round(float(referential_quantity2), int(self.float_point_precision))) if referential_quantity2 else '',
                    str(unit2) if unit2 else '',
                    str(unit2_name) if unit2_name else '',
                    str(round(float(referential_quantity3), int(self.float_point_precision))) if referential_quantity3 else '',
                    str(unit3) if unit3 else '',
                    str(unit3_name) if unit3_name else '',
                    str(referential_working_hours) if referential_working_hours else '',
                    '',
                    ''
                ])

                # Set yellow background for standard quantities row
                for i in range(ref_child.columnCount()):
                    ref_child.setBackground(i, QtGui.QColor("#FFFFD0"))

                # Create a child item for produced quantities
                prod_child = QTreeWidgetItem([
                    'Produced',
                    '',
                    str(round(float(quantity1), int(self.float_point_precision))) if quantity1 else '',
                    str(unit1) if unit1 else '',
                    str(unit1_name) if unit1_name else '',
                    str(round(float(quantity2), int(self.float_point_precision))) if quantity2 else '',
                    str(unit1) if unit1 else '',
                    str(unit2_name) if unit2_name else '',
                    str(round(float(quantity3), int(self.float_point_precision))) if quantity3 else '',
                    str(unit2) if unit2 else '',
                    str(unit3_name) if unit3_name else '',
                    str(working_hours) if working_hours else '',
                    '',
                    str(batch) if batch else '',
                ])

                # Set orange background for produced quantities row
                for i in range(prod_child.columnCount()):
                    prod_child.setBackground(i, QtGui.QColor("#FFD8C0"))

                # Add children to parent
                parent_item.addChild(ref_child)
                parent_item.addChild(prod_child)

                # Add parent item to tree
                self.ui.produced_materials_tree.addTopLevelItem(parent_item)
                # Auto expand the parent item
                parent_item.setExpanded(True)
                
            #fetch produced materials warehouses
            warehouses = self.database_operations.fetchWarehouses()
            produced_materials = self.database_operations.fetchManufactureProcessProducedMaterials(self.manufacture_process_id)
            for produced_material in produced_materials:
                material_id = produced_material['material_id']
                material_name = produced_material['material_name']
                input_warehouse_id = produced_material['warehouse']
                input_warehouse_name = produced_material['warehouse_name']
                # Add new row
                current_row = self.ui.material_warehouses_table.rowCount()
                self.ui.material_warehouses_table.insertRow(current_row)


                # Fill table cells
                self.ui.material_warehouses_table.setItem(current_row, 0, QTableWidgetItem(str(material_name)))
                self.ui.material_warehouses_table.setItem(current_row, 1, QTableWidgetItem(str(material_id)))
                # Create combobox for warehouse column
                warehouse_combobox = QComboBox()    
                if warehouses:
                    for warehouse in warehouses:
                        warehouse_id = warehouse['id']
                ...
                x = warehouse_combobox.findData(input_warehouse_id)
                warehouse_combobox.setCurrentIndex(warehouse_combobox.findData(input_warehouse_id))
                self.ui.material_warehouses_table.setCellWidget(current_row, 2, warehouse_combobox)

            # fetch composition materials
            manufacture_materials = self.database_operations.fetchManufactureProcessMaterials(self.manufacture_process_id)
            for manufacture_material in manufacture_materials:
                id = manufacture_material['id']
                manufacture_id = manufacture_material['manufacture_id']
                composition_item_id = manufacture_material['composition_item_id']
                standard_quantity = manufacture_material['standard_quantity']
                required_quantity = manufacture_material['required_quantity']
                unit = manufacture_material['unit']
                unit_cost = manufacture_material['unit_cost']
                row_type = manufacture_material['row_type']
                warehouse_id = manufacture_material['warehouse_id'] or ''
                warehouse_name = manufacture_material['warehouse_name'] or ''
                warehouse_quantity = manufacture_material['warehouse_quantity'] or ''
                warehouse_account_id = manufacture_material['warehouse_account_id'] or ''
                pulled_quantity = manufacture_material['pulled_quantity']
                shortage = manufacture_material['shortage']
                currency = manufacture_material['currency']
                code = manufacture_material['code']
                material_name = manufacture_material['material_name']
                unit_name = manufacture_material['unit_name']
                currency_name = manufacture_material['currency_name']

                try:
                    cost = round((float(unit_cost)*float(required_quantity)),4)
                except Exception as e:
                    cost = ''
                    print("Error calculating cost. "+str(e))
                # composition qtreewidget
                tree_row = QTreeWidgetItem(
                    [str(composition_item_id), str(code), str(material_name), str(standard_quantity), str(unit), str(unit_name),
                        str(round(float(required_quantity), int(self.float_point_precision))), str(unit), str(unit_name), 
                        str(round(float(unit_cost), int(self.float_point_precision))) if unit_cost and str(unit_cost).lower() != 'none' else '',
                        str(round(float(cost), int(self.float_point_precision))) if cost != '' else '',
                        str(currency),
                        str(currency_name), str(warehouse_id), str(warehouse_name), str(warehouse_account_id), str(warehouse_quantity), '', '', str(pulled_quantity), str(shortage)])

                if shortage and str(row_type).lower()=='parent':
                    for i in range(self.ui.composition_tree.columnCount()):
                        tree_row.setBackground(i, light_red_color)
                elif not shortage and str(row_type).lower()=='parent':
                    for i in range(self.ui.composition_tree.columnCount()):
                        tree_row.setBackground(i, light_green_color)

                if str(row_type).lower()=='parent':
                    self.ui.composition_tree.addTopLevelItem(tree_row)

                elif str(row_type).lower()=='child':
                    x = self.ui.composition_tree.topLevelItemCount()
                    for row in range(x):
                        child_composition_item = self.ui.composition_tree.topLevelItem(row)
                        child_composition_item_id = child_composition_item.text(0)  # Assuming the first column is column 0
                        if child_composition_item_id is not None and child_composition_item_id == str(composition_item_id):
                            child_composition_item.addChild(tree_row)
            

            # Fetch machines
            machines_data = self.database_operations.fetchManufactureProcessMachines(self.manufacture_process_id)
            for machine in machines_data:
                machine_id = machine['machine_id']
                machine_name = machine['machine_name'] 
                machine_mode = machine['mode_name'] 
                machine_mode_id = machine['mode_id'] 
                machine_duration = machine['duration'] 
                machine_exclusive = machine['exclusive'] 
                machine_exclusive_percent = machine['exclusive_percent'] 
                machine_duration_hours = round(int(machine_duration) / 60, int(self.float_point_precision))
                ...
                # Add new row
                current_row = self.ui.machines_table.rowCount()
                self.ui.machines_table.insertRow(current_row)
                
                # Fill table cells
                self.ui.machines_table.setItem(current_row, 0, QTableWidgetItem(str(machine_id)))
                self.ui.machines_table.setItem(current_row, 1, QTableWidgetItem(str(machine_name)))
                self.ui.machines_table.setItem(current_row, 2, QTableWidgetItem(str(machine_mode_id)))
                self.ui.machines_table.setItem(current_row, 3, QTableWidgetItem(str(machine_mode)))
                self.ui.machines_table.setItem(current_row, 4, MyTableWidgetItem(str(machine_duration_hours), int(machine_duration_hours)))
                self.ui.machines_table.setItem(current_row, 5, MyTableWidgetItem(str(machine_duration), int(machine_duration)))
                self.ui.machines_table.setItem(current_row, 6, QTableWidgetItem(str(machine_exclusive)))
                self.ui.machines_table.setItem(current_row, 7, QTableWidgetItem(str(machine_exclusive_percent)))
                self.ui.machines_table.setItem(current_row, 8, QTableWidgetItem(str(account_id)))
                self.ui.machines_table.setItem(current_row, 9, QTableWidgetItem(str(account_name)))
                self.ui.machines_table.setItem(current_row, 10, QTableWidgetItem(str(opposite_account_id)))
                self.ui.machines_table.setItem(current_row, 11, QTableWidgetItem(str(opposite_account_name)))

                # Add custom delegate for exclusive column
                delegate = MyCustomTableCellDelegate(col=6, row=None, condition=[None, 5, 'No'])
                self.ui.machines_table.setItemDelegate(delegate)

            # Fill manufacture details table with costs
            self.ui.manufacture_details_table.setRowCount(0)  # Clear existing rows
            
            cost_items = [
                (self.language_manager.translate('RAW_MATERIALS_COST'), str(round(float(composition_materials_cost), int(self.float_point_precision)))),
                (self.language_manager.translate('MACHINES_OPERATION_COST'), str(round(float(machines_operation_cost), int(self.float_point_precision)))),
                (self.language_manager.translate('SALARIES_COST'), str(round(float(salaries_cost), int(self.float_point_precision)))),
                (self.language_manager.translate('EXPENSES'), str(round(float(expenses_cost), int(self.float_point_precision)))),
            ]
            total_cost = 0
            
            for row, (cost_type, cost_value) in enumerate(cost_items):
                try:
                    cost = float(cost_value) if cost_value and str(cost_value).lower() != 'none' else 0
                except ValueError:
                    cost = 0
                
                total_cost += cost
                self.ui.manufacture_details_table.insertRow(row)
                self.ui.manufacture_details_table.setItem(row, 0, QTableWidgetItem(cost_type))
                self.ui.manufacture_details_table.setItem(row, 1, QTableWidgetItem(f"{round(float(cost), int(self.float_point_precision)):.{self.float_point_precision}f}"))
                self.ui.manufacture_details_table.setItem(row, 2, QTableWidgetItem(str(currency)))
                self.ui.manufacture_details_table.setItem(row, 3, QTableWidgetItem(str(currency_name)))
            
        # Add total cost row
        total_row = self.ui.manufacture_details_table.rowCount()
        self.ui.manufacture_details_table.insertRow(total_row)
        self.ui.manufacture_details_table.setItem(total_row, 0, QTableWidgetItem(self.language_manager.translate('TOTAL_MANUFACTURE_COST')))
        self.ui.manufacture_details_table.setItem(total_row, 1, QTableWidgetItem(f"{round(float(total_cost), int(self.float_point_precision)):.{self.float_point_precision}f}"))
        self.ui.manufacture_details_table.setItem(total_row, 2, QTableWidgetItem(str(currency)))
        self.ui.manufacture_details_table.setItem(total_row, 3, QTableWidgetItem(str(currency_name)))
    ...
    def saveJournalEntry(self, manufacture_process_id, commit=True):
        ...
        if self.ui.generate_journal_entry.isChecked():
            ...
            #     for resource_id, data in self.salaries_costs_dict.items():
            #         value = data[0]
            #         currency = data[1]
            #         account_id = data[2]
                ...
                for material_id, ingredient_warehouses in self.materials_costs_dict.items():
                    for ingredient_warehouse in ingredient_warehouses:
                        ...
                        if warehouse_account:
                            ...
                            messagebox_result = win32api.MessageBox(None, message, "تنبيه", MB_YESNO);
                            if (messagebox_result == IDYES):
                                pass
                            elif (messagebox_result == IDNO):
    ...
    def addMaterialToOutputInvoice(self,output_invoice_id,manufacture_currency):
        ...
        for row in range(self.ui.composition_tree.topLevelItemCount()):
            ...
            invoice_item_id = self.database_operations.addInvoiceMaterial(
                ...
                gifts_value=0,
                gifts_discount=0,
                warehouse_id=None,  # Will be pulled from composition mater ials
                cost_center_id=None,
    ...
    def addToWarehouse(self, manufacture_process_id, manufacture_currency):
        ...
        for i in range(root.childCount()):
            ...
            default_unit = item.child(1).text(3)
