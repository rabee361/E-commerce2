...
class Ui_AddMaterialToWarehouse_Logic(QDialog):
    ...
    def saveMaterial(self, window):
        ...
        if material_data is None:
            ...
            unit_price = unit_price if unit_price else ''
            if not quantity:
                win32api.MessageBox(0,"الكمية لا يمكن أن تكون فارغة","خطأ")
            else:
                unit = self.ui.unit_combobox.currentData()
                ...
                material_move_id = self.database_operations.moveMaterial(material_id=material_id, quantity=quantity, move_unit=unit, from_warehouse='', from_account_warehouse=from_account_warehouse, to_account_warehouse=from_account_warehouse, to_warehouse=self.warehouse_id, origin_type='transfer', origin_id=invoice_item_id, cost=total_cost, currency=currency_id, record_only=True, record_journal_entry=True)
