...
from Ui_NewReadyInvoice import Ui_NewReadyInvoice
from LanguageManager import LanguageManager
from PyQt5.QtCore import QTranslator

class Ui_NewReadyInvoice_Logic(object):
    def __init__(self, sqlconnector):
        super().__init__()
        self.sqlconnector = sqlconnector
        self.database_operations = DatabaseOperations(sqlconnector)
        self.ui = Ui_NewReadyInvoice()
    ...
    def setProductFields(self):
        ...
        if (str(type(product_data)) != "<class 'NoneType'>"):
            ...
            product_unit_price_usd = round(float(float(product_unit_price_sp) / float(exchange_rate)), 3)
            ...
            product_price_after_discount_usd = round(
                float(float(product_unit_price_usd) * (100 - float(product_discount)) / 100), 3)
    ...
    def calculateProductFields(self):
        ...
        if currency=='ل.س':
            ...
            product_unit_price_sp = round(float(float(product_unit_price_usd) * float(exchange_rate)), 3)

            product_unit_price_after_discount_sp = round(
                float(float(product_unit_price_sp) * (100 - float(product_discount)) / 100), 3)
        else:
            pass
    ...
    def calculateTotalInvoiceValue(self):
        ...
        total_sp_after_discount = round(float(float(total_sp) * (100 - float(discount)) / 100),3)
        total_usd_after_discount = round(float(float(total_usd) * (100 - float(discount)) / 100),3)
