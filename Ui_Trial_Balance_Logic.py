...
from PyQt5.QtWidgets import QDialog, QTreeWidgetItem , QSizePolicy, QVBoxLayout, QGridLayout, QCheckBox, QFrame, QPushButton
from Ui_DataPicker_Logic import Ui_DataPicker_Logic
from Colors import light_red_color, white_color
from DatabaseOperations import DatabaseOperations
from Ui_Trial_Balance import Ui_Trial_Balance
from PyQt5.QtCore import Qt, QDate
from LanguageManager import LanguageManager
from PyQt5.QtCore import QTranslator
from PyQt5.QtGui import QIcon

class Ui_Trial_Balance_Logic(QDialog):
    def __init__(self, sql_connector):
        super().__init__()
        self.sql_connector = sql_connector
        self.database_operations = DatabaseOperations(sql_connector)
        self.ui = Ui_Trial_Balance()
    ...
    def calculate(self, targeted_currency=None, from_date=None, to_date=None):
        ...
        def addDataToTable(dictionary, parent_account=None): #inner function
            for key, values in dictionary.items():
                    ...
                    items_already_in_tree = self.ui.results_tree.findItems(str(account_id),Qt.MatchExactly | Qt.MatchRecursive, 0)
                    
                    # Get or create tree item
                    if len(items_already_in_tree) > 0:
                        tree_item = items_already_in_tree[0]
                    else:
                        tree_item = QTreeWidgetItem([str(account_id), str(account_name), "", "", "", "", ""])
                        if parent_account:
                    ...
                    for value in values:
                        ...
                        if isinstance(value, list):
                            ...
                            # Aggregate creditor values
                            for creditor_key, creditor_value in creditors_dict.items():
                                currency_id = creditor_key[0]
                                currency_name = creditor_key[1]
                                currency_key = (currency_id, currency_name)
                                if currency_key not in currency_dict:
                                    currency_dict[currency_key] = {"creditor_value": 0, "debtor_value": 0}
                                currency_dict[currency_key]["creditor_value"] += creditor_value

                            # Aggregate debtor values
                            for debtor_key, debtor_value in debtors_dict.items():
                                currency_id = debtor_key[0]
                                currency_name = debtor_key[1]
                                currency_key = (currency_id, currency_name)
                                if currency_key not in currency_dict:
                                    currency_dict[currency_key] = {"creditor_value": 0, "debtor_value": 0}
                                currency_dict[currency_key]["debtor_value"] += debtor_value

                            # Create tree items for each currency
                            for currency_key, values in currency_dict.items():
                                currency_id, currency_name = currency_key
                                creditor_value = values["creditor_value"]
                                debtor_value = values["debtor_value"]

                                child_item = QTreeWidgetItem(
                                    ["", "", str(currency_id), str(currency_name), str(debtor_value),
                                     str(creditor_value), str(abs(creditor_value -  debtor_value))])

                                # Set background color to red and text color to white if debtor and creditor values are not the same
                                if creditor_value != debtor_value:
                                    child_item.setBackground(4, QBrush(light_red_color))
                                    child_item.setBackground(5, QBrush(light_red_color))
    ...
    def calculateLogic(self, targeted_account, from_date, to_date, targeted_currency, unified_currency, targeted_currency_name, unified_exchange_date, exchange_date, distinct_currency, data_sources):
        ...
        for journal_entry_item in journal_entry_items:
            ...
            if journal_entry_type == 'debtor':
                debtors_dict[currency_key] = debtors_dict.get(currency_key, 0) + value

            elif journal_entry_type == 'creditor':
                creditors_dict[currency_key] = creditors_dict.get(currency_key, 0) + value
            ...
            for child_key, child_values in child_result_dict.items():
                for child_value in child_values:
                    if isinstance(child_value, list):
                        ...
                        for child_debtors_dict_key, child_debtors_dict_value in child_debtors_dict.items():
                            if child_debtors_dict_key in debtors_dict:
                                debtors_dict[child_debtors_dict_key] = debtors_dict.get(child_debtors_dict_key, 0) + child_debtors_dict_value
                            else:
                                debtors_dict[child_debtors_dict_key]=child_debtors_dict_value

                        for child_creditor_dict_key, child_creditors_dict_value in child_creditors_dict.items():
                            if child_creditor_dict_key in creditors_dict:
                                creditors_dict[child_creditor_dict_key] = creditors_dict.get(child_creditor_dict_key, 0) + child_creditors_dict_value
                            else:
                                creditors_dict[child_creditor_dict_key]=child_creditors_dict_value
        ...
        final_accounts = self.database_operations.fetchAccounts(final_account=targeted_account_id)
        for final_account in final_accounts:
            ...
            final_result_dict = self.calculateLogic(final_account_id, from_date, to_date, targeted_currency, unified_currency, targeted_currency_name, unified_exchange_date, exchange_date, distinct_currency, data_sources)

            if final_result_dict:
                if key in result_dict:
                    result_dict[key].append(final_result_dict)
                else:
                    result_dict[key] = [final_result_dict]

            for final_key, final_values in final_result_dict.items():
                for final_value in final_values:
                    if isinstance(final_value, list):
                        final_creditors_dict, final_debtors_dict=final_value
                        for final_debtors_dict_key, final_debtors_dict_value in final_debtors_dict.items():
                            if final_debtors_dict_key in debtors_dict:
                                debtors_dict[final_debtors_dict_key] = debtors_dict.get(final_debtors_dict_key, 0) + final_debtors_dict_value
                            else:
                                debtors_dict[final_debtors_dict_key]=final_debtors_dict_value

                        for final_creditor_dict_key, final_creditors_dict_value in final_creditors_dict.items():
                            if final_creditor_dict_key in creditors_dict:
                                creditors_dict[final_creditor_dict_key] = creditors_dict.get(final_creditor_dict_key, 0) + final_creditors_dict_value
                            else:
                                creditors_dict[final_creditor_dict_key]=final_creditors_dict_value
