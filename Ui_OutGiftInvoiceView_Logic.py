...
from Ui_OutGiftInvoiceView import Ui_OutGiftInvoiceView
from Ui_Payments_Logic import Ui_Payments_Logic


class Ui_OutGiftInvoiceView_Logic(object):
    ...
    def initialize(self):
        ...
        self.fetchOutGiftInvoice()

    def openPaymentsWindow(self):
        Ui_Payments_Logic(self.sqlconnector, self.invoice_id).showUi()
        self.setPaid()

    def fetchOutGiftInvoice(self):
        invoice_data = self.database_operations.fetchGiftsOutInvoice(self.invoice_id)
        ...
        for item in invoice_data:
            ...
            total_sp = total_sp + price_sp
            total_usd = total_usd + price_usd
            total_sp_after_discount = total_sp_after_discount + price_sp_after_discount
            total_usd_after_discount = total_usd_after_discount + price_usd_after_discount
    ...
    def setPaid(self):
        invoice_data = self.database_operations.fetchDirectOutInvoice(self.invoice_id)
        paid = invoice_data[0][3]
    ...
    def exportToExcel(self):
        ...
        if currency == 'ل.س':
            ...
            worksheet.write("B" + str(excel_row_count + 2), self.ui.result_after_discount_usd_input.text(),
                            green_format)
