...
from Ui_NewSampleInvoice import Ui_NewSampleInvoice


class Ui_NewSampleInvoice_Logic(object):
    def __init__(self, sqlconnector):
        super().__init__()
        self.sqlconnector = sqlconnector
        self.database_operations = DatabaseOperations(sqlconnector)
        self.ui = Ui_NewSampleInvoice()

    def showUi(self):
        window_newsampleinvoice = QDialog()
        self.ui.setupUi(window_newsampleinvoice)
        self.initialize(window_newsampleinvoice)
        window_newsampleinvoice.exec()
    ...
    def initialize(self, window):
        ...
        self.fetchProductManufactureAvgs()
        ...
        self.ui.products_combobox.currentIndexChanged.connect(lambda: self.fetchProductManufactureAvgs())
    ...
    def fetchProductManufactureAvgs(self):
        ...
        if (str(type(product_data)) != "<class 'NoneType'>"):
            ...
            costs = self.database_operations.fetchProductManufactureAverageUntilDate(product_id, date)
            try:
                unit_cost_sp = round(float(costs[0][0]),6)
            except:
                unit_cost_sp = 0.0
            self.ui.price_sp_input.setText(str(unit_cost_sp))

            try:
                unit_cost_usd = round(float(costs[0][1]),6)
            except:
                unit_cost_usd = 0.0
            self.ui.price_usd_input.setText(str(unit_cost_usd))
    ...
    def setProductFields(self):
        ...
        if (str(type(product_data)) != "<class 'NoneType'>"):
            ...
            product_price_after_discount_sp = round(
    ...
    def saveInvoice(self):
        ...
        if self.ui.items_table.rowCount() > 0 and client != '':
            new_out_invoice_id = self.database_operations.addOutInvoice(number, type, client, direct_client, payment,
                                                                        statement, paid, total_discount, currency,
                                                                        exchange, date)
            for row in range(self.ui.items_table.rowCount()):
                ...
                self.database_operations.addOutInvoiceItem(new_out_invoice_id, product_id, quantity, price_sp,
                                                           price_usd, discount, price_sp_after_discount,
