...
class DatabaseOperations(object):
    ...
    @check_permission('manufacture', 'r')
    def fetchLastManufatureProcess(self) -> None:
        print("DATABASE> Fetch last manufacture process")
        query = """SELECT
                m.name AS material_name,
                mf.date_col AS manufacture_date,
                mp.quantity1 AS quantity,
                w.name AS warehouse_name
            FROM
                manufacture mf
            JOIN
                manufacture_produced_materials mp ON mf.id = mp.manufacture_id
            JOIN
                materials m ON mp.material_id = m.id
            JOIN
                warehouseslist w ON mf.warehouse = w.id
            ORDER BY
                mf.date_col DESC
            LIMIT 1;
            """
    ...
    @check_permission('hr_requests', 'r')
    ...
    def fetchLastMaterialMovement(self):
        ...
        # Step 1: Resolve the codenames for source and destination warehouses
        codename_query = """
            SELECT
                (SELECT codename FROM warehouseslist WHERE id = mm.source_warehouse) AS source_codename,
                (SELECT codename FROM warehouseslist WHERE id = mm.destination_warehouse) AS destination_codename
            FROM
                material_moves mm
            LIMIT 1;
        """

        self.cursor.execute(codename_query)
        codenames = self.cursor.fetchone()
        self.sqlconnector.conn.commit()

        if not codenames:
            print("No codenames found for warehouses!")
            return None

        source_table = codenames['source_codename']
        destination_table = codenames['destination_codename']

        # Step 2: Construct the dynamic SQL query using the resolved table names
        dynamic_query = f"""
            SELECT
                mm.quantity,
                mm.date_col,
            ...
            LEFT JOIN `{source_table}` sw ON m.material_id = sw.material_id AND m.id = sw.material_id
            LEFT JOIN warehouseslist ws ON mm.source_warehouse = ws.id
            LEFT JOIN warehouseslist dw ON mm.destination_warehouse = dw.id
            ORDER BY
                mm.date_col DESC
            LIMIT 1;
        """
        ...
        self.cursor.execute(dynamic_query)
        rows = self.cursor.fetchall()
        self.sqlconnector.conn.commit()
        return rows
